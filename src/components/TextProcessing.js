...
class TextProcessing extends Component {
  ...
  handleRun() {
    ...
    let res = value
      .replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g, '') // убираем все лишнее из
      // текста(запятые, скобочки и т.д. ...)
      .replace(/\s+/g, ' ') //заменяем множественные пробелы или абзацы на 1 пробел
      .trim() //убираем пробелы в начале и конце строки
      .split(' '); //делаем из набора слов массив для дальнейшей работы

    let newRes = map(res, this.word); // итерируемся по каждому слову в массиве и меняем
    // последнию букву с последней с помощью вынесенной функции
    this.setState({ resultValue: newRes.join(' ') }); //преобразуем массив слов в текст
    //и кладем в state результативного текста
  }
